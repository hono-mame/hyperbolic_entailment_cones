...
angleChild :: [Float] -> [Float] -> Float
angleChild parentEmb childEmb = 0.0

angleParent :: [Float] -> [Float] -> Float -> Float
angleParent parentEmb childEmb alpha = 3.0 * alpha
...
neuralDTSBuilderNoPickle :: IO (String -> String -> Float)
neuralDTSBuilderNoPickle = do
            ...
            in case (mpIdx, mcIdx) of
                (Just pIdx, Just cIdx) ->
                    case (M.lookup pIdx embMap, M.lookup cIdx embMap) of
                        (Just pEmb, Just cEmb) ->
                            let a1 = angleChild pEmb cEmb
                                a2 = angleParent pEmb cEmb alpha
                                score = a1 - a2
...
main :: IO ()
main = do
    oracle <- neuralDTSBuilderNoPickle
